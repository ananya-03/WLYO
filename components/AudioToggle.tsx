...
export function AudioToggle({ enabled, onToggle }: AudioToggleProps) {
  return (
    <motion.button
      ...
      aria-label={enabled ? "Mute audio" : "Turn up audio"}
      className={`
        ...
        ${enabled
          ? "bg-acid/20 border-acid text-acid shadow-[0_0_15px_var(--acid)]"
          : "bg-ink/50 border-lavender/50 text-lavender hover:border-lavender"
        }
      `}
    ...
    >
      ...
      <span>{enabled ? "MUTE" : "TURN UP"}</span>
    </motion.button>
  );
}
